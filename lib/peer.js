...
//Create socket io connection with SPS server
 ...
 */
Peer.prototype.JoinSwarmChannel = function(swarmchannel,stream, socket) {
		var self = this;
		if(stream)
			self.SwarmStream = stream;
		if(socket){
			this.SwarmSocket = socket;
		}else{
			this.SwarmSocket = this._openswarmsocket();
		}
		if(!stream){
		...
		this.SwarmSocket.on("joined channel",function(channel,ismoderator)  {
			...
			if(self.onJoinedChannel)
				self.onJoinedChannel(channel,ismoderator);
		});
		this.SwarmSocket.on("Call",function(peer) {
			if(self.SwarmStream)
				self.call(peer, self.SwarmStream);
		}); 
		this.SwarmSocket.emit("join channel",swarmchannel);
};
/**
